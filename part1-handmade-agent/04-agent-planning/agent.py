...
压缩、记忆与上一篇相同。这篇在记忆里加一份计划（步骤列表），用 set_plan / mark_step 维护。

要点（与文章同一套叫法）：
- 计划 = 记忆里的步骤列表；步骤 status 只有 pending / done / failed
- 计划工具：set_plan、mark_step；业务工具：get_weather、search_hotels、book_hotel
- 没有计划时先 set_plan，再调业务工具
- 每轮只推进一个 pending 步骤
- 参数错了重试（不改计划）；步骤做不到再 mark_step(failed) + set_plan（重规划）
"""
...
# 假天气：10-01 下雨，10-02 晴。用来看预订时会不会按记忆里的天气选房型。
WEATHER = {
...
def get_weather(city: str, date: str) -> dict:
    ...
    result = {"city": city, "date": date, **hit}
    if memory is not None:
        memory.remember_weather(result)
    return result
...
def run(goal: str, max_turns: int = 12, token_budget: int = 24_000) -> str:
    global memory
    memory = Memory(goal)
    messages = [
